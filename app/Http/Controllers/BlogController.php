...
class BlogController extends Controller
{
    ...
    public function blog($title){
        ...
        Twitter::setTitle(''.$Settings->sitename.' - '.$Settings->welcome.'');
        Twitter::setSite(''.$Settings->twitter.'');
        $Category = DB::table('category')->limit(10)->InRandomOrder()->get();
        $Blog = DB::table('blog')->where('slug',$title)->get();
        $Popular = DB::table('blog')->paginate(8);
     
        foreach($Blog as $value){
            $SEO = '
            <title> '.$value->title.' | Edition Investments | premier supplier of quality wood based building.</title>
            ...
            <link rel="canonical" href="'.url('/news/'.$value->slug.'').'" />
            ';
            ...
            $Related = DB::table('blog')->where('cat',$value->cat)->limit(10)->get();
            // echo count($Related);
            // die();
                    ...
                    $Author = DB::table('users')->where('name',$newName)->get();
                    return view('front.news-single',compact('SEO','Related','Category','page_title','Blog','page_name','Comments','Author','Popular'));
        }
    ...
    public function add_comment(Request $request){
        ...
        $comments->blog_id=$request->post_id;
        $comments->save();
    ...
    public function blogCat($cat){
        ...
        foreach ($Categories as $key => $value) {
            ...
            if($Blog->isEmpty()){
                ...
                $page_name = 'Sorry There are No Blogs Written For That Category';
            }else{
        ...
        return view('front.newscat',compact('page_title','Blog','page_name'));
    }
    }
